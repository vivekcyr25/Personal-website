...
const Workspace: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <PortfolioWizard isOpen={isWizardOpen} onClose={() => setIsWizardOpen(false)} onDeploy={handleDeploy} />
      ...
      {!hasIdentity ? (
            ...
            <div className="liquid-glass p-12 text-center group border border-theme-border hover:border-theme-primary/30 transition-all duration-700">
              <div className="flex flex-col items-center gap-8">
                ...
                <div className="space-y-4">
                  <h2 className="font-orbitron text-4xl font-black tracking-widest text-white uppercase">INITIALIZE_NEURAL_LINK</h2>
                  <p className="font-space-mono text-sm text-white/40 max-w-md mx-auto leading-relaxed">
                ...
                <div className="flex gap-4 w-full max-w-sm">
                  <button 
                    ...
                    className="flex-1 bg-theme-primary/10 border border-theme-primary/30 text-theme-primary py-5 rounded-2xl font-orbitron font-black tracking-[0.3em] hover:bg-theme-primary hover:text-dark-bg transition-all shadow-[0_0_30px_rgba(var(--theme-primary-rgb),0.2)] flex items-center justify-center gap-3 active:scale-95"
                  >
                    START_INITIALIZATION <Zap size={20} />
                  </button>
                </div>
              </div>
            </div>
        ...
        <>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-2">
            <div>
              <h2 className="font-orbitron text-2xl font-black tracking-widest text-white neural-glow">PORTFOLIO_COMMAND_CENTER</h2>
              <p className="font-space-mono text-[10px] text-white/40 uppercase tracking-[0.3em] mt-1">Manage Deployments & Neural Configurations</p>
            </div>
            <button 
              onClick={() => { playClick(); setIsWizardOpen(true); }}
              onMouseEnter={playHover}
              className="bg-theme-primary text-dark-bg px-6 py-3 rounded-xl font-orbitron text-[10px] font-bold tracking-widest uppercase hover:scale-105 transition-all shadow-[0_0_20px_rgba(var(--theme-primary-rgb),0.3)] flex items-center gap-2"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="liquid-glass p-4 border border-theme-border hover:border-theme-secondary/30 transition-all">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-space-mono text-white/40 uppercase">Global Sync</p>
                  <p className="text-xl font-orbitron text-theme-secondary">99.8%</p>
                </div>
            ...
            <div className="liquid-glass p-4 border border-theme-border hover:border-theme-primary/30 transition-all">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-space-mono text-white/40 uppercase">Cluster Load</p>
                  <p className="text-xl font-orbitron text-theme-primary">{telemetry.load}%</p>
                </div>
            ...
            <div className="liquid-glass p-4 border border-theme-border hover:border-theme-accent/30 transition-all">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-space-mono text-white/40 uppercase">Node Latency</p>
                  <p className="text-xl font-orbitron text-theme-accent">{telemetry.latency}ms</p>
                </div>
            ...
            <div className="liquid-glass p-4 border border-theme-border hover:border-white/30 transition-all">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-space-mono text-white/40 uppercase">Brain Link</p>
                  <p className="text-xl font-orbitron text-white">ACTIVE</p>
                </div>
                ...
                <div className="space-y-4">
                    <div className="p-4 bg-white/5 border border-white/10 rounded-xl flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-orbitron text-[10px] font-bold text-white tracking-widest">Semantic Indexing</p>
                          <p className="font-space-mono text-[8px] text-white/40 uppercase">RAG Vector DB update</p>
                        </div>
                      </div>
                      <span className="text-theme-primary font-space-mono text-[9px] animate-pulse">RUNNING...</span>
                    </div>
                   <div className="p-4 bg-black/40 border border-white/5 rounded-xl flex items-center justify-between opacity-50">
                     <div className="flex items-center gap-3">
                       ...
                       <div>
                         <p className="font-orbitron text-[10px] font-bold text-white tracking-widest">Asset Optimization</p>
                         <p className="font-space-mono text-[8px] text-white/40 uppercase">Pending deployment</p>
                       </div>
                     </div>
                     <span className="text-white/40 font-space-mono text-[9px]">QUEUED</span>
                   </div>
                </div>
              </HologramPanel>
                  ...
                  {recentActivity.map((act, i) => (
                    <div key={i} className="flex items-center justify-between p-3 bg-white/5 border border-white/5 rounded-xl group hover:border-theme-primary/30 transition-all cursor-crosshair">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-[9px] font-orbitron font-bold text-white">{act.target}</p>
                          <p className="text-[7px] font-space-mono text-white/30 uppercase">{act.type} // {act.time}</p>
                        </div>
                      </div>
                      <span className={`text-[7px] font-space-mono px-2 py-1 rounded bg-black/40 ${act.status === 'SUCCESS' || act.status === 'VERIFIED' || act.status === 'ACTIVE' ? 'text-theme-secondary' : 'text-theme-primary'}`}>
                        {act.status}
                      </span>
                    </div>
                  ))}
